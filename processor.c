...
signed zeroext(uint16_t offset) {
	return offset & 0x0000ffff;
}

signed int signext(uint16_t offset) { 
	if (0x8000 & offset) {
		// extend to 32 bits, extended digits should be 1
		return offset | 0xffff0000;
	}
	else {
		// extend to 32 bits, extended digits should be 0
		return zeroext(offset);
	}
}
